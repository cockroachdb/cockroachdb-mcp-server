...
// ensureSelectOnly recursively rejects DML smuggled through CTEs, bracket
...
// walkTableExprReadOnly rejects non-SELECT statements smuggled through
...
// validateShowStatement enforces that stmt is some form of SHOW. The thin
// guardrail matches the design doc's principle of reinforcing CRDB privileges
// rather than reimplementing them: CRDB itself rejects SHOW statements the
// connected role lacks privileges for (e.g. SHOW JOBS requires admin).
func validateShowStatement(stmt tree.Statement) error {
	if !strings.HasPrefix(stmt.StatementTag(), "SHOW") {
		return errors.Newf("only SHOW statements are allowed, got %s", stmt.StatementTag())
	}
	return nil
}
