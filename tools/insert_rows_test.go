...
func TestInsertRows(t *testing.T) {
	t.Run("a valid INSERT passes through and reports rows affected", func(t *testing.T) {
	...
	t.Run("RETURNING routes through Query and returns the rows", func(t *testing.T) {
		...
		h := newWriteHandlers(fq)
		res, _, err := h.insertRows(context.Background(), &mcp.CallToolRequest{},
			InsertRowsParams{Statement: "INSERT INTO users VALUES (1, 'a') RETURNING id"})
		require.NoError(t, err)
	...
	for _, tc := range rejections {
		t.Run("rejects "+tc.name, func(t *testing.T) {
			fq := &fakeQuerier{}
			_, _, err := newWriteHandlers(fq).insertRows(context.Background(), &mcp.CallToolRequest{},
